...
(function ($, window) {
  ...
  $.fn.bootpag = function (options) {
    ...
    function createPageArray(start = 1, end = 1) {
      var tempArray = [];
    ...
    return this.each(function () {
      ...
      var $bootpag, me = $(this),
        ...
        currentPageIndex = me.data('pageindex'),
        totalPage = me.data('pagecount');

      settings.maxVisible = 5;
      ...
      if (settings.maxVisible % 2 === 1 && settings.maxVisible < totalPage) {
        ...
        let min = settings.maxVisible - mid;
        let max = totalPage - mid;


        if (currentPageIndex <= min) {
          let start = 1, end = settings.maxVisible;
          p = p.concat(createPageArray(start, end));
        }
        else if (currentPageIndex > min && currentPageIndex <= max){
          let start = currentPageIndex - mid, end = start + (settings.maxVisible - 1);
          p = p.concat(createPageArray(start, end));
        }
        else {
          let start = totalPage - (settings.maxVisible - 1), end = totalPage;
          p = p.concat(createPageArray(start, end));
        }
      }
      ...
      if (settings.maxVisible % 2 === 0 && settings.maxVisible < totalPage) {
        ...
        if (currentPageIndex <= min) {
          ...
          let start = currentPageIndex - (mid - 1), end = currentPageIndex + mid;
          p = p.concat(createPageArray(start, end));
        }
        else {
          let start = totalPage - (settings.maxVisible - 1), end = totalPage;
      ...
      p.push('</ul>');
      me.find('nav.nav-bootpag').remove();
      me.append(p.join(''));
